...
eXcell_ra.prototype.setValue=function(val){
	...
	if (val){
		...
		if (this.grid._RaSeCol[this.cell._cellIndex]){
			...
			z.setValue("0");
			if (arguments[1]) z.cell.wasChanged=true;
...
function eXcell_txt(cell){
	...
	this.edit=function(){
		...
		if (_isFF){
			...
			z_ff.select=function(){this.firstChild.select(); }
...
eXcell_cp.prototype.setValue=function(val){
	this.setCValue("<div style='width:100%;height:"+(this.cell.offsetHeight-2)+";background-color:"+(val||"")
		+";border:0px;'>&nbsp;</div>",
		val);
}
//#}
...
eXcell_dyn.prototype.setValue=function(val){
	...
	this.setCValue("<div style='position:relative;padding-right:2px; width:100%;overflow:hidden;'><img src='"+this.grid.imgURL+""+img
